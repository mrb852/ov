...
and checkExp ftab vtab (exp : In.Exp)
   ...
   *)
    ...
    | In.Negate (e, pos) =>
      ...
      in case t of
        ...
        | _ => raise Error("Expression is not an int", pos)
      end
    ...
    | In.Not(e, pos) =>
      ...
      in case t of
        ...
        | _ => raise Error("Expression is not a bool", pos)
      end
  ...
  (* TODO: TASK 2: Add case for Filter.  Quite similar to map, except that the
    ...
    | In.Filter (f, arr_exp, _, pos)
      => let val (arr_type, arr_exp_dec) = checkExp ftab vtab arr_exp
             ...
             val (f', f_ret_type, f_arg_type) =
               case checkFunArg (f, vtab, ftab, pos) of
                   (f', ret, [a1]) => (f', ret, a1) (* passed function should take exactly 1 argument *)
                 | (_,  ret, args) =>
                   raise Error ("Filter: incompatible function type of "
                                ^ In.ppFunArg 0 f ^ ":" ^ showFunType (args, ret), pos)
         in if f_ret_type = Bool then
              if elem_type = f_arg_type
              then (Array elem_type,
                    Out.Filter (f', arr_exp_dec, elem_type, pos))
              else raise Error ("Filter: input array type does not match the passed in function's argument type." ^
                                "Input array type: " ^ ppType elem_type ^ "." ^
                                "Passed in function argument" ^ ppType f_arg_type , pos)
            else raise Error ("Filter: function argument is a function that does not return bool. Instead it returns"
                                ^ ppType f_ret_type, pos)
         end
  ...
  | In.ArrCompr _ => raise Error("ArrCompr not implemented in TypeChecker", (387, 0))
...
and checkFunArg (In.FunName fname, vtab, ftab, pos) =
  ...
  | checkFunArg (In.Lambda (tp, params, body, funpos), vtab, ftab, pos) =
    let
      val f = In.FunDec ("<lambda>", tp, params, body, funpos)
    in
      let
        val Out.FunDec(_, tp', params', body', pos') = checkFunWithVtable (f, vtab, ftab, pos)
        val param_types = map (fn Param(s, t) => t) params'
      in
        (Out.Lambda (tp', params', body', pos'), tp', param_types)
      end
    end
