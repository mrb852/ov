...
(*********************************************************)
   ...
   *)
    ...
    | And (e1, e2, pos) =>
        let val t1 = newName "and_L"
            ...
            val endLabel = newName "and_endLabel"
            val code1 = compileExp e1 vtable t1
            val code2 = compileExp e2 vtable t2
        in [Mips.MOVE (place, "0")] @ code1 @ [Mips.BEQ (t1, "0", endLabel)] @
           code2 @ [Mips.MOVE(place, t2), Mips.LABEL endLabel]
        end

    | Or (e1, e2, pos) =>
        let val t1 = newName "or_L"
            val t2 = newName "or_R"
            val endLabel = newName "or_endLabel"
            val code1 = compileExp e1 vtable t1
            val code2 = compileExp e2 vtable t2
        in [Mips.ADDI (place, "0", "1")] @ code1 @ [Mips.BEQ (t1, place, endLabel)] @
           code2 @ [Mips.MOVE(place, t2), Mips.LABEL endLabel]
        end
    ...
    | Scan (farg, acc_exp, arr_exp, tp, pos) =>
        let val size_reg = newName "size_reg" (* size of input/output array *)
            ...
            val loop_map0 =
                case getElemSize tp of
                    One => Mips.LB(res_reg, elem_reg, "0")
                           :: applyFunArg(farg, [acc_reg, res_reg], vtable, res_reg, pos)
                           @ [ Mips.ADDI(elem_reg, elem_reg, "1") ]
                  | Four => Mips.LW(res_reg, elem_reg, "0")
                            :: applyFunArg(farg, [acc_reg, res_reg], vtable, res_reg, pos)
                            @ [ Mips.ADDI(elem_reg, elem_reg, "4") ]
  ...
  (* TODO: TASK 2: Add case for Filter.
    ...
    | Filter (farg, arr_exp, elem_type, pos) =>
        let val size_reg = newName "size_reg" (* size of input/output array *)
            ...
            val addr_reg = newName "addr_reg" (* address of element in new array *)
            val i_reg = newName "i_reg"
            val i2_reg = newName "i2_reg" (* counts elements in new_arr *)
            val init_regs = [ Mips.ADDI (addr_reg, place, "4")
                            , Mips.MOVE (i_reg, "0")
                            , Mips.MOVE (i2_reg, "0")
                            , Mips.ADDI (elem_reg, arr_reg, "4") ]
            ...
            val loop_beg = newName "loop_beg"
            val loop_end = newName "loop_end"
            val loop_continue = newName "loop_continue"
            val tmp_reg = newName "tmp_reg"
            val loop_header = [ Mips.LABEL (loop_beg)
                              , Mips.SUB (tmp_reg, i_reg, size_reg)
                              , Mips.BGEZ (tmp_reg, loop_end) ]
            ...
            (* check if funArg succeeded *)
            val if_funarg_true = [ Mips.BEQ(res_reg, "0", loop_continue) ]

            (* if (funArg)predicate succeeds *)
            (* load res_reg = old_arr[i] *)
            val then_loadfrom_oldarr =
              case getElemSize elem_type of
                One => [ Mips.LB (res_reg, elem_reg, "0") ]
                | Four => [ Mips.LW (res_reg, elem_reg, "0") ]

            (* store res_reg in new_arr[i2] *)
            val storein_newarr =
                case getElemSize elem_type of
                    One =>  [ Mips.SB (res_reg, addr_reg, "0") ]
                  | Four => [ Mips.SW (res_reg, addr_reg, "0") ]
                  (* increment new_arr counter, and address of new_arr *)
                  @ [   Mips.ADDI (addr_reg, addr_reg, makeConst (elemSizeToInt (getElemSize elem_type))),
                        Mips.ADDI (i2_reg, i2_reg, "1") ]

            val loop_footer =
                [ Mips.LABEL(loop_continue)
                , Mips.ADDI (i_reg, i_reg, "1")
                , Mips.ADDI (elem_reg, elem_reg, makeConst (elemSizeToInt (getElemSize elem_type))) (* incr old_arr *)
                , Mips.J loop_beg
                , Mips.LABEL loop_end
                , Mips.SW(i2_reg, place, "0")
                ]
        in arr_code
           @ get_size
  ...
  (* TODO TASK 5: add case for ArrCompr.
   ...
   expression using map and filter, then run compileExp on that. *)
    | ArrCompr _ => raise Error("ArrCompr not implemented in CodeGen", (830, 1))
  ...
  and applyFunArg (FunName s, args, vtable, place, pos) : Mips.Prog =
     ...
     (* TODO TASK 3:
        ...
        'place' register.
      *)
    | applyFunArg (Lambda(tp, params, body, funpos), args, vtable, place, pos) =
      let
        (* Bind parameters of the lambda to registers *)
        fun bindParams vtab (Param(s, _) :: params) (a :: args) pos =
            let
              val vtab' = SymTab.bind s a vtab
              val vtab'' = bindParams vtab' params args pos
            in vtab'' end
          | bindParams vtab [] [] _ = vtab
          | bindParams _ _ _ pos = raise Error("Non-matching Lambda params and args.", pos)
        val vtab' = bindParams vtable params args pos
        val body_reg = newName "body_reg"
        val body = compileExp body vtab' body_reg
      in body @ [Mips.MOVE (place, body_reg)] end
