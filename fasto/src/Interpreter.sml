...
fun evalExp ( Constant (v,_), vtab, ftab ) = v
  ...
  | evalExp( Not(e, pos), vtab, ftab ) =
        let val res = evalExp(e, vtab, ftab)
        in case res of
              BoolVal (value) => BoolVal(not(value))
            | _               => raise Error("Value is not a bool", pos)
        end
  ...
  | evalExp ( Negate(e, pos), vtab, ftab ) =
      ...
      in case r of
        ...
        | _            => raise Error("Could not convert value to integer\n", pos)
      end

  | evalExp ( And(e1, e2, pos), vtab, ftab ) =
       (case evalExp(e1, vtab, ftab) of
          BoolVal true =>
            (case evalExp(e2, vtab, ftab) of
              BoolVal b2 => BoolVal b2
            | _          => raise Error("Second operand not boolean", pos))
        | BoolVal false => BoolVal false
        | _             => raise Error("First operand not boolean", pos))
...
and evalFunArg (FunName fid, vtab, ftab, callpos) =
  ...
  | evalFunArg (Lambda (rettype, params, body, fpos), vtab, ftab, callpos) =
    let
      val f = FunDec("<lambda>", rettype, params, body, fpos)
    in
